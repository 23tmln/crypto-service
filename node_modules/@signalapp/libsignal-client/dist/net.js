...
import { UnauthenticatedChatConnection, AuthenticatedChatConnection, ProvisioningConnection, } from './net/Chat.js';
...
function withSuffix(keys, suffix) {
    return keys.map((key) => `${key}.${suffix}`);
}
...
const PROD_REMOTE_CONFIG_KEYS = ['chatPermessageDeflate.prod'];
...
export class Net {
     ...
     * @param options.abortSignal an {@link AbortSignal} that will cancel the connection attempt.
     */
    async connectProvisioning(listener, options) {
        return ProvisioningConnection.connect(this.asyncContext, this._connectionManager, listener, options);
    }
